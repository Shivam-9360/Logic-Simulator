  ...
  */
  ...
  // physical devices
    connection.start().then(function () {
        var uniqueId = 0;
      // register hardware input component
      $s.registerDevice('Hardware (IN)', function (device) {
          ...
          device.$ui.on('deviceAdd', function () {
              connection.invoke('SendInput', `device${uniqueId}`).catch(function (err) {
                  return console.error(err.toString());
              });
              connection.on(`GetInput-device${uniqueId}`, function (input) {
                  device.getOutputs()[0].setValue(input == '1' ? 1 : null);
              });
          });
          device.$ui.on('deviceRemove', function () {
              device.getOutputs()[0].setValue(null);
